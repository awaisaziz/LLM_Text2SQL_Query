...
def DeepSeek_generation(prompt, model=MODEL):
    """
    Use DeepSeek to generate SQL or reasoning outputs.
    """
...
def DeepSeek_debug(prompt, model=MODEL):
    """
    Use DeepSeek to debug and fix SQL queries.
    """
    response = client.chat.completions.create(
        model=model,
        messages=[
    ...
    print(f"Debug API raw response: {response.choices[0].message.content.strip()}")
    return response.choices[0].message.content.strip()
...
if __name__ == '__main__':
    ...
    for index, row in val_df.iterrows():
        ...
        # if index >= 100:      # generate 100 sql queries
        #     break
        ...
        while schema_links is None:
            try:
                schema_links = DeepSeek_generation(
        ...
        while classification is None:
            try:
                classification = DeepSeek_generation(
        ...
        if '"EASY"' in predicted_class:
            ...
            while SQL is None:
                try:
                    SQL = DeepSeek_generation(easy_prompt_maker(row['question'], row['db_id'], schema_links))
                    print(f"EASY SQL: {SQL}")
                except:
                    time.sleep(3)
                    pass
            ...
            while SQL is None:
                try:
                    SQL = DeepSeek_generation(medium_prompt_maker(row['question'], row['db_id'], schema_links))
                    print(f"NON-NESTED SQL: {SQL}")
                except:
                    time.sleep(3)
                    pass
            # try:
        ...
        #     try:
        #         debugged_SQL = DeepSeek_debug(debuger(row['question'], row['db_id'], SQL)).replace("\n", " ")
        #         print(f"debugged_SQL: {debugged_SQL}")
        #     except:
        #         time.sleep(3)
        #         pass
